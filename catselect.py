...
def next3_story():
    ...
    story3_image_rect = story3_image.get_rect(topleft=[0,0])
    ...
    while True:
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                ...
                # 이벤트 타입이 마우스버튼을 누르기 이벤트인지 확인 
                if story3_image_rect.collidepoint(event.pos):
                    return "select_cat"

def select_cat():
    select_image = pygame.image.load("C:\\Users\\jjh48\\OneDrive\\바탕 화면\\파이썬응용\\pygame-2--main\\bg\\next_back_grond.jpg")
    select_image = pygame.transform.scale(select_image, (width, height))
    screen.blit(select_image,(0,0))
    
    music_albom_image = pygame.image.load("C:\\Users\\jjh48\\OneDrive\\바탕 화면\\파이썬응용\\pygame-2--main\\music_albom_picture.png")
    screen.blit(music_albom_image,(50,50))
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if(mouse_x>=430 and mouse_x <= 430+300 and mouse_y >= 30 and mouse_y<= 30+300):
                    cat_x+=1
...
while running:
    ...
    elif current_state == "select_cat":
        current_state = select_cat()
